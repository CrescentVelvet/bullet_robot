...
class PPO:
    def __init__(self, env, outdir):
        ...
        self.actor = FeedForwardNN(self.obs_dim, self.act_dim).to(self.device) # actor网络初始化,actor
    ...
    def _init_hyperparameters(self): # 定义默认的超参数
        ...
        self.max_timesteps_per_episode = 1600 # 运行一个episode的最大步数,episode是agent在环境中根据某个策略执行一系列action到结束的完整回合
    ...
    def learn(self, total_timesteps):
        ...
        while t_so_far < total_timesteps: # 循环设定总步数
            ...
            avg_rew = self.avg_reward_per_episode(batch_rtgs, batch_lens) # 求每个episode的平均reward
            print('[{}] Average episodic reward: {}'.format(i_learn, avg_rew))
            t_so_far += np.sum(batch_lens) # 循环步数加上一个batch的步数
            batch_obs, batch_acts, batch_log_probs, batch_rtgs = self.randomize(batch_obs, batch_acts, batch_log_probs, batch_rtgs) # 打乱顺序
            for i in range(self.epochs): # 循环epochs次
                for k in range(0, batch_rtgs.shape[0], self.minibatch_size): # 从0到rtgs长度循环,每次增加minibatch
            ...
            if i_learn % 100 == 0: # while循环100次保存模型一次
                self.save(i_learn)
            i_learn +=1
    ...
    def evaluate(self, batch_obs, batch_acts): # 计算log概率和obs的value,输入obs和act,输出log概率和obs的预测值(V-value)
        ...
        mean = self.actor(batch_obs).squeeze() # 这三句代码
    ...
    def avg_reward_per_episode(self, batch_rtgs, batch_lens): # 求每个episode平均reward函数
        episodic_rewards = []
        for i, ep_len in enumerate(batch_lens): # 将batch_lens数组转换为索引
            total_time_so_far = int(np.sum(batch_lens[:i])) # 计算到目前为止的总步数
            episodic_rewards.append(batch_rtgs[total_time_so_far:total_time_so_far+ep_len].sum().item()) # 从总步数到下一个episode长度的rtgs求和,加入reward
        return np.mean(episodic_rewards) # 返回reward的平均值
    def randomize(self, batch_obs, batch_acts, batch_log_probs, batch_rtgs): # 打乱顺序函数
        idx = np.random.randint(0, batch_rtgs.shape[0], batch_rtgs.shape[0]) # 返回从0到rtgs长度的共rtgs长度个随机数
        batch_obs = batch_obs[idx] # 打乱顺序
        batch_acts = batch_acts[idx] # 打乱顺序
        batch_log_probs = batch_log_probs[idx] # 打乱顺序
        batch_rtgs = batch_rtgs[idx] # 打乱顺序
        return batch_obs, batch_acts, batch_log_probs, batch_rtgs
    def save(self, name): # 模型保存函数
        torch.save({
            'actor': self.actor.state_dict(), # 保存actor模型
            'critic': self.critic.state_dict() # 保存critic模型
        }, os.path.join(self.outdir, '{}train_{}.pth'.format(self.now, name)))
    def load(self, path): # 模型读取函数
        checkpoint = torch.load(path)
        self.actor.load_state_dict(checkpoint['actor']) # 读取actor模型
        self.critic.load_state_dict(checkpoint['critic']) # 读取critic模型