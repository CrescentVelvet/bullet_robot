...
class RobotEnv(gym.Env):    
    def __init__(self, render=False): # 初始化函数
        ...
        if render: # 连接物理引擎
            self.physicsClient = p.connect(p.GUI) # 仿真可视化
        else:
            self.physicsClient = p.connect(p.DIRECT) # 仿真不可视化
    ...
    def reset(self): # 重置环境函数
        ...
        p.changeDynamics(planeId,-1,lateralFriction = 0.6,spinningFriction = 0.3,rollingFriction = 0.0001) # 设置地面摩擦力(旋转摩擦,横向摩擦,滚动摩擦)        
    ...
    def _compute_observation(self): # 计算环境观测值(object)
        states = [p.getJointState(self.robot_urdf, self.jointNameToID_robot['joint_arm_left']), # 获取当前机器人关节角度与角速度信息
        ...
        cube_pos, cube_orn = p.getBasePositionAndOrientation(self.robot_urdf) # 返回世界坐标系中的位置[x,y,z]和姿态[x,y,z,w]
        cube_euler = p.getEulerFromQuaternion(cube_orn) # 将姿态四元数转换为欧拉角[yaw,pitch,roll]
        linear, angular = p.getBaseVelocity(self.robot_urdf) # 返回世界坐标系中的速度[x,y,z]和角速度[yaw,pitch,roll]
    ...
    def _compute_reward(self): # 计算动作奖励量(float)
        ...
        # reward = p.getLinkState(self.robot_urdf, self.linkNameToID_robot['body_head'])[4][2] # 奖励 = 头部z坐标
        ...
        falling = 0 # 防止倒下
        if p.getLinkState(self.robot_urdf, self.linkNameToID_robot['body_head'])[4][2] < 0.5:
            falling = 0.5
    ...
    def _set_controler(self, input_action): # 设置关节控制器        
        ...
        control_mode = p.VELOCITY_CONTROL # 控制模式
        ...
        p.setJointMotorControl2(bodyIndex=self.robot_urdf,
