...
class RobotEnv(gym.Env):    
    ...
    def _compute_reward(self): # 计算动作奖励量(float)
        robot_pos, robot_orn = p.getBasePositionAndOrientation(self.robot_urdf) # 返回世界坐标系中的位置[x,y,z]和姿态[x,y,z,w]        
        robot_euler = p.getEulerFromQuaternion(robot_orn) # 将姿态四元数转换为欧拉角[yaw,pitch,roll]
