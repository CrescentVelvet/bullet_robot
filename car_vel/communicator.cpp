    ...
    while(true) {
        ...
        if (receiveSocket[t].state() == QUdpSocket::BoundState && receiveSocket[t].hasPendingDatagrams()) {
            ...
            for(int i = 0; i < commands.command_size(); i++) {
                ...
                RobotSpeed rs(command.velocity_x(), command.velocity_y(), command.velocity_r(), command.use_dir());
                commandBuffer[t].robotSpeed[command.robot_id()] = rs;
                //qDebug()<<"czkdebug:: medusa kick"<<command.kick()<<command.power();
            }
...
void Communicator::sendCommand(int team, int id) {
    ...
    if (flat || chip) {
        AutoShootFit::instance()->getKickerMes(team, id, chip);
    }
...
//    qDebug() << "SENDCMD FLAT:" << flat <<" team:" << team << " id:" << id;
