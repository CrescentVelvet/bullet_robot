...
outdir='/home/zjunlict-vision-1/Desktop/bullet_robot/model' # 设置模型保存地址
print('------环境构建完成---开始构建模型------')
model = PPO(env, outdir) # 建立PPO模型
total_timesteps = 1000000 # 循环设定总步数
model.learn(total_timesteps=total_timesteps) # 模型训练
model.save('wyf-pleg') # 保存模型
print('------模型构建完成---开始仿真训练------')
# del model # 删除模型
# model = PPO.load(outdir) # 读取模型
